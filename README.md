...
- 🌐 **可视化界面** - 基于 Streamlit 的美观现代化 Web 界面
...
├── run.py                       # 根启动入口，转发到 bootstrap 启动器
├── app.py                       # 根级兼容 Streamlit 入口
├── video_ai_suite/              # 正式拆分后的源码目录
│   ├── bootstrap/               # 启动项：环境检查、模型检查、下载、安装、启动
│   ├── backend/                 # 后端服务项：路径、缓存、Token 等公共能力
│   └── streamlit_app/           # Streamlit 项：正式入口、状态初始化、页面主体
...
- **QQ**: 153115068（请备注来意）
- **邮箱**: 153115068@qq.com
- **Issues**: [GitHub Issues](https://github.com/JucieOvo/Frame-by-frame-AI-parser_by_JucieOvo/issues)
