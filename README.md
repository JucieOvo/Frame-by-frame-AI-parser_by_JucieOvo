...
[快速安装](#-快速安装) • [功能详解](#-功能详解) • [使用流程](#-使用流程) • [常见问题](#-常见问题)
...
- 🖼️ **多模态图像分析** - 支持阿里云 Qwen-VL、硅基流动API和本地 Ollama 模型
...
- 🔄 **模型名称实时更新** - 自动获取提供商支持的最新模型列表
- 🛡️ **OLLAMA性能优化** - 长视频解析时自动重置VLM，防止无法响应
...
#### 1. 启动程序
- **方式一（推荐）**: 双击运行 `run.bat` 文件
- **方式二（源码运行）**: 在命令行中运行 `python run.py`
- **注意事项**: 首次运行会自动安装依赖和下载模型，请耐心等待

#### 2. 上传视频文件
- **支持格式**: MP4, AVI, MOV, MKV, FLV, WMV 等常见视频格式
- **文件大小**: 建议不超过 2GB，大文件处理时间较长
- **上传方式**: 通过界面拖拽或点击上传按钮

#### 3. 配置分析参数
- **模型选择**: 
  - 阿里云 DashScope（推荐，速度快，质量高）
  - 硅基流动API（推荐，支持多厂商模型）
  - Ollama（本地部署，免费但配置复杂）
- **分析类型**: 
  - 完整分析报告（全面详细）
...
#### 4. 开始分析
- **自动处理**: 程序自动检测场景并提取关键帧
- **并行处理**: 抽帧、语音识别、图像分析三线程并行
- **进度显示**: 实时显示分析进度和预估剩余时间

#### 5. 查看分析结果
- **结果展示**: 结构化显示视频内容分析结果
- **导出功能**: 支持导出为文本文件或PDF格式
- **历史记录**: 自动保存分析记录，便于后续查看
...
**验证命令**:
```bash
echo %DASHSCOPE_API_KEY%  # 应该显示你的阿里云 API Key
echo %SILICONFLOW_API_KEY%  # 应该显示你的硅基流动 API Key
```
</details>

<details>
<summary><b>Q: 程序运行内存占用过高？</b></summary>
...
**建议配置**: 16GB 内存可流畅运行大多数视频分析任务
</details>

<details>
<summary><b>Q: 分析结果不准确怎么办？</b></summary>
...
#### 🐛 问题修复列表
- **新增模型提供商**: 新增对硅基流动API的支持,允许用户使用 硅基流动API 提供的模型作为VLM和LLM
- **模型名称实时更新**: 新增模型名称实时更新功能,用户可查看提供商支持的最新模型，无需手动更新
- **对OLLAMA作为提供商时的性能优化**: 在长视频解析时会在一定时间后重置OLLAMA上的VLM以防止出现无法响应的情况，对未知条件下的异常抛出自动修正，自行重启OLLAMA上的VLM模型
